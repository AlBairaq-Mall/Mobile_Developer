...
// /// Model for a delivery order returned by:
// /// GET /api/delivery/orders
// /// GET /api/delivery/orders/{id}
// ///
// /// Fields are parsed flexibly — any field not confirmed in the actual
// /// API response is kept nullable and defaults to null rather than
// /// inventing data. Mapping should be updated once a real response is received.
// class DeliveryOrderModel {
//   final String id;

//   /// Order number shown to users (e.g. "ORD-1045")
...
//   // Customer info (may be nested under a 'user' or 'customer' key)
//   final String customerName;
//   final String customerPhone;

//   // Delivery address (may be a nested object or flat string)
...
//     // Customer may be nested under 'user', 'customer', or flat fields
//     final userMap = json['user'] ?? json['customer'] ?? {};
//     final customerName = userMap is Map
//         ? JsonParser.string(userMap['name'] ?? userMap['full_name'])
//         : JsonParser.string(json['customer_name']);

//     final customerPhone = userMap is Map
//         ? JsonParser.string(userMap['phone'] ?? userMap['mobile'])
//         : JsonParser.string(json['customer_phone']);

//     // Address may be nested or a flat string
//     final addressRaw = json['address'] ?? json['delivery_address'];
//     final String address;
//     if (addressRaw is Map) {
//       final parts = <String>[
//         JsonParser.string(addressRaw['city']),
//         JsonParser.string(addressRaw['district']),
//         JsonParser.string(addressRaw['street']),
//         JsonParser.string(addressRaw['address']),
//       ].where((s) => s.isNotEmpty).toList();
//       address = parts.join(' - ');
//     } else {
//       address = JsonParser.string(addressRaw);
//     }
