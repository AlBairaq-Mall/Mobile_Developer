import 'package:bhm_supermarket/app/di/dependency_injection.dart';
import 'package:bhm_supermarket/core/widgets/app_page_header.dart';
import 'package:flutter/material.dart';
import '../../../app/theme/app_colors.dart';
import 'package:bhm_supermarket/features/products/widgets/product_unit_picker_sheet.dart';
...
class _AdminOffersScreenState extends State<AdminOffersScreen> {
  Future<List<ProductUnitSelection>?> _pickOfferProducts(
    BuildContext context,
  ) async {
    return showModalBottomSheet<List<ProductUnitSelection>>(
      context: context,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const AppPageHeader(title: 'إدارة العروض'),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      OutlinedButton.icon(
                        onPressed: () async {
                          final result = await _pickOfferProducts(context);

                          if (result == null || result.isEmpty) return;

                          final products = result
                              .map((selection) => selection.toOfferJson())
                              .toList();

                          debugPrint('Offer products: $products');

                          // لاحقًا نرسل products إلى POST /api/offers
                        },
