...
class AdminAdsScreen extends StatefulWidget {
  ...
  @override State<AdminAdsScreen> createState() => _AdminAdsScreenState();
}

class _AdminAdsScreenState extends State<AdminAdsScreen> {
  // TODO: GET /api/admin/ads
  final _ads = <_AdItem>[
    _AdItem(id:'1', title:'خصم رمضان 30%', desc:'جميع المشروبات', active:true,  start:'2026-07-01', end:'2026-07-30'),
    _AdItem(id:'2', title:'عروض الصيف',    desc:'منتجات الألبان',  active:false, start:'2026-08-01', end:'2026-08-31'),
  ];
            ...
            child: ListTile(
              leading: Container(
                ...
                decoration: BoxDecoration(
                  color: ad.active ? AppColors.primary.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
                  ...
                  Switch(
                    ...
                    onChanged: (v) {
                      setState(() => _ads[i] = _AdItem(
                        id: ad.id, title: ad.title, desc: ad.desc,
                        active: v, start: ad.start, end: ad.end));
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: TextField(decoration: const InputDecoration(labelText: 'تاريخ البداية', prefixIcon: Icon(Icons.calendar_today)))),
                const SizedBox(width: 10),
                Expanded(child: TextField(decoration: const InputDecoration(labelText: 'تاريخ النهاية', prefixIcon: Icon(Icons.event)))),
              ],
            ),
            const SizedBox(height: 12),
            OutlinedButton.icon(
...
class _AdItem {
  ...
  final bool active;
  const _AdItem({required this.id, required this.title, required this.desc,
